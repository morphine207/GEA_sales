...
//import {saveViewerSettingsAPI,loadViewerSettingsAPI} from "../engine/parameters/viewerApi.js";
import { Screenshotdialog } from './Screenshotdialog.js';
import { ShowPipelineDialog } from './PipelineDialog.js';
import {ShowSaveSettingDialog} from './savesettingsdialog.js'
...
export class Website
{
    ...
    InitToolbar ()
    {
      ...
      //  AddSeparator (this.toolbar, ['only_full_width', 'only_on_model']);
       // AddButton (this.toolbar, 'save_settings', Loc ('Save Viewer Settings'), ['only_full_width', 'only_on_model'], () => {
        //    console.log("Viewer Objekt:", this.viewer);
        //    this.viewer.mainModel.EnumerateMeshesAndLines((mesh) => {
        //        console.log("Mesh ID:", mesh.id, "Visible:", mesh.visible);
        //    });
        //    saveViewerSettingsAPI(this.viewer);
        //    console.log("Button (Save Routin) Pressed!");
        //});

        // Button zum Laden der Einstellungen in die Toolbar einfügen
       // AddButton(this.toolbar, "snapshot", Loc("Create Screenshots"), ["only_full_width", "only_on_model"], () => {
       //     Screenshotdialog (this.viewer);

        //});
        AddSeparator (this.toolbar, ['only_full_width', 'only_on_model']);
        AddButton (this.toolbar, 'save_settings', Loc ('save settings'), ['only_full_width', 'only_on_model'], () => {
            console.log("Viewer Objekt:", this.viewer);
            this.viewer.mainModel.EnumerateMeshesAndLines((mesh) => {
                console.log("Mesh ID:", mesh.id, "Visible:", mesh.visible);
            });
            ShowSaveSettingDialog(this.viewer);
            console.log("Button (Save Routin) Pressed!");
        });

        // Button zum Laden der Einstellungen in die Toolbar einfügen
        AddButton(this.toolbar, "snapshot", Loc("edit pipline"), ["only_full_width", "only_on_model"], () => {
            ShowPipelineDialog();

        });

              // Button zum Laden der Einstellungen in die Toolbar einfügen
              AddButton(this.toolbar, "snapshot", Loc("upload screenshots"), ["only_full_width", "only_on_model"], () => {
                Screenshotdialog (this.viewer);
